...
if( $hidop == 'list'){
    ...
    $sqlgetcount = 'select * from '._TABLE_PAGE.' where project_id='.$project_id;    
    ...
    // 取得所有的block列表
    $sql = 'select * from '._TABLE_BLOCK.' where project_id='.$project_id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $rows = array();
    ...
    // 取得要修改的page資料
    $sql = 'select * from '._TABLE_PAGE.' where id='.$id.' and project_id='.$project_id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    $xajax=new xajax();
    $xajax->registerFunction("ajax_block_update");
    ...
    // 取得Page區塊的範本
    $sql = 'select * from '._TABLE_PAGE_EXAMPLE.' where enable=\'1\'';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    $sql = 'delete from '._TABLE_PAGE.' where id='.$id.' limit 1';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');
...
} elseif( $hidop == 'exportpage' ){
        
    $debuglines[] = php_pagedir_export($id,$project_id);
