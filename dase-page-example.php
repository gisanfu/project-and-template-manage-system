...
$thisfilename = 'dase-page-example.php';
$pagedir = 'page_example';
...
if( $hidop == 'list'){
    ...
    $sqlgetcount = 'select * from '._TABLE_PAGE_EXAMPLE;
    ...
    // 這裡並不會去抓Block的資料，因為範本是不建立關連的
    
    // 取得要修改的page資料
    $sql = 'select * from '._TABLE_PAGE_EXAMPLE.' where id='.$id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    ...
    $sql = 'delete from '._TABLE_PAGE_EXAMPLE.' where id='.$id.' limit 1';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');
