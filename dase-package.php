...
if( $_GET["hidop"] != '' ){
  $hidop = $_GET["hidop"];
  $id    = $_GET["id"];
  $package_company_id = $_GET["package_company_id"];
} elseif( $_POST["hidop"] != '' ){
  $hidop = $_POST["hidop"];
  $id    = $_POST["id"];
  $package_company_id = $_POST["package_company_id"];
} else {
  echo 'no arg error';
  exit;
}
...
if( $hidop == 'list'){
    ...
    $sql = 'select * from '._TABLE_PACKAGE_COMPANY.' where id='.$package_company_id;    
    ...
    $sqlgetcount = 'select * from '._TABLE_PACKAGE.' where package_company_id='.$package_company_id;    
    ...
    $page->setViewList($thisfilename."?hidop=list&package_company_id=".$package_company_id.'&', 'test');  
    ...
    $smarty->assign("package_company_id",$package_company_id);
    ...
    if( $debug !== 1 ) 
        $smarty->assign('redir',$thisfilename.'?hidop=list&package_company_id='.$package_company_id);
    $smarty->assign('debuglines',$debuglines);
    $smarty->assign('targetpost',$thisfilename);
    ...
    $sql = 'select * from '._TABLE_PACKAGE.' where id='.$id.' and package_company_id='.$package_company_id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    $sql = 'delete from '._TABLE_PACKAGE.' where id='.$id.' limit 1';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');
