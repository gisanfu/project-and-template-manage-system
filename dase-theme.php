...
if( $hidop == 'list'){
    ...
    $sqlgetcount = 'select * from '._TABLE_THEME.' where project_id='.$project_id;    
    ...
    $sql = 'select * from '._TABLE_THEME.' where id='.$id.' and project_id='.$project_id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    $sql = 'delete from '._TABLE_THEME.' where id='.$id.' limit 1';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');
...
} elseif( $hidop == 'exportthemefile' ){
        
    $debuglines[] = php_themefile_export($project_id);
