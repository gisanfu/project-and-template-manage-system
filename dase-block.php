...
if( $hidop == 'list'){
    ...
    $sqlgetcount = 'select * from '._TABLE_BLOCK.' where project_id='.$project_id;    
    ...
    $sql_values['headname'] = $_POST["headname"];
    ...
    // 取得要修改的Blocks
    $sql = 'select * from '._TABLE_BLOCK.' where id='.$id.' and project_id='.$project_id;
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    // 取得Block區塊範本
    $sql = 'select * from '._TABLE_BLOCK_EXAMPLE.' where enable=\'1\'';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');   
    $row  = array();
    $row = mysql_fetch_array($result,MYSQL_ASSOC);
    ...
    $sql = 'delete from '._TABLE_BLOCK.' where id='.$id.' limit 1';
    $result = mysql_query($sql);
    include('pages/check-db-result.php');
...
} elseif( $hidop == 'exportblock' ){
        
    $debuglines[] = php_blockdir_export($id,$project_id);
